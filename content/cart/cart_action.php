<?php 
	...
	elseif(isset($_GET['CapNhat']))
	{
		foreach($_GET as $masp=>$soluong)
		{
			if(($soluong > 0) and (is_numeric($soluong)))
			{
				...
				$_SESSION['cart'][$masp]['thanhtien'] = number_format($soluong * $_SESSION['cart'][$masp]['giamoi'], 0, ',', '.');
	...
	elseif(isset($_GET['ThanhToan']))
	{
		...
		if($hinhthucGH == 'Giao hàn tiếp kiệm')
			$ship = '20000';
		...
		foreach($_SESSION['cart'] as $sp)
		{
			...
			$thanhtien = number_format($soluong * $_SESSION['cart'][$masp]['giamoi'], 0, ',', '.');
			...
			$soluong_ton = $kho[0]['soluong_ton'];
			$tonkho = $soluong_ton - $soluong;
			$update=DP::run_query("UPDATE kho SET soluong_ton=? WHERE masp=?",[$tonkho,$masp],3);
		}
