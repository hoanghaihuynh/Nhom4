...
if($str == 'Log In')
 ...
 ?>
<button type="button" class="btn btn-success" data-toggle="modal" data-target="<?php echo $id_cart; ?>" style='width:120px;height:42px;font-size:14px;width:100%'>THANH TOÁN</button>
...
<div class="modal fade" id="message_cart">
  <div class="modal-dialog">
      ...
      <div class="modal-header">
          ...
          <h3>Bạn cần đăng nhập để thanh toán.</h3>
      </div>

      <div class='modal-footer'>
        <center>
          <a href="<?php echo $level.'login.php'; ?>" class='DangNhap'>Đăng Nhập Ngay</a>
        </center>
      </div>
      </div>
  </div>
...
<script language='javascript'>
	...
	function HinhThucGH(obj){
		...
		for(var i=1; i < option.length; i++){
			if(option[i].selected){
				str += option[i].value;
				if(option[i].value=='Giao hàng nhanh')
					ship = '50.000';
				else if(option[i].value=='Giao hàng tiết kiệm')
					ship = '20.000';
			}
		}
